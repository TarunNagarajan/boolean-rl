...
class DQNAgentSeq:
    def __init__(self, vocab_size, action_size, seed, hidden_size=64, embedding_dim=32, learning_rate=5e-4, gamma=0.99, tau=1e-3, buffer_size=int(1e5), batch_size=64, update_every=4):
        ...
        self.qnet_policy = QNetworkSeq(vocab_size, action_size, seed, embedding_dim, hidden_size).to(self.device)
        self.qnet_target = QNetworkSeq(vocab_size, action_size, seed, embedding_dim, hidden_size).to(self.device)
    ...
    def act(self, state):
        ...
        state_tensor = torch.from_numpy(state).long().unsqueeze(0).to(self.device)
    ...
    def learn(self, experiences):
        ...
        next_states = torch.from_numpy(np.vstack([e.next_state for e in experiences])).long().to(self.device)
