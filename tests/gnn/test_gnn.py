...
def test_DQN_GNN(num_test_episodes=10):
    ...
    try:
        ...
        print(f"{COLOR_RED}Error: checkpoint_gnn.pth not found. Please ensure the training script has been run and the file exists.{COLOR_RESET}")
        return
    except Exception as e:
        print(f"{COLOR_RED}Error loading model: {e}{COLOR_RESET}")
        return

    print(f"\n{COLOR_LAVENDER}--- Starting GNN Model Testing ---{COLOR_RESET}")
